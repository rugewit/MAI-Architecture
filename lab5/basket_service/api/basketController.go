...
func BasketRegisterRoutes(r *fiber.App, basketService *services.BasketService, userService *services.UserService,
	...
	r.Get("/", basketController.GetBaskets)
	r.Get("/:id", basketController.GetBasket)
	r.Put("/:id", basketController.UpdateBasket)
	//routes.DELETE("/:id", basketController.DeleteBasket)
	r.Post("/add/:idbasket/:idproduct", basketController.AddProduct)
	r.Delete("/remove/:idbasket/:idproduct", basketController.RemoveProduct)
}
